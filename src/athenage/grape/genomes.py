...
class Genome:
    ...
    def add_codon(self,val:int, rule_used:int):
    ...
    def add_tail(self, n_codons:int):
        for i in range(n_codons):
            self.codons.append(random.randint(0,self.codon_size))
    
    def random_fill(self,genome_length:int):
    ...
    def consumed(self, rule_used:int) -> bool:
        ...
        # if self.next_read >= len(self.codons):
        #     return True
        # else:
        #     return False

    def reset_map_index(self):
    ...
    def set_codon(self, codon_idx:int, value:int, invalid:bool):
    ...
    def crossover_onepoint(self, genome2, pos1, pos2) -> list:
